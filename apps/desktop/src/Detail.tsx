...
import { ArrowRight, Check, Copy, Download, ExternalLink, FileInput, FlaskConical, Folder, Layers3, ScanSearch, GitBranch, Github, History, Loader2, Pencil, Rocket, RotateCcw, ShieldCheck, Sparkles, Star, Trash2, TriangleAlert, ZoomIn } from 'lucide-react';
import type { Installation, Item, ItemDetail, Provider, ProviderId, PublishJob, Revision, Snapshot, Trial } from '../../../packages/protocol/schema';
...
export function Detail({ jobs, detail, snapshot, providers, sameTitle, installations, onAction, onToggleInstall, refresh, perform, onSelect, onSetup, onCollection, onMadeFrom }: Props) {
  ...
  // A source is the material an analysis read: it has what was made from it instead of trials, approval and installs.
  const isSource = item.kind === 'source';
  const made = snapshot.items.filter(i => i.origin?.itemId === item.id && !i.deletedAt);
  ...
  const tabs = isSource ? ['overview', 'made', 'content', 'history'] : ['overview', 'content', 'history', 'trials', ...(installable ? ['installs'] : [])];
  const tabName = (t: string) => isSource ? ({ made: 'made', content: 'original' } as Record<string, string>)[t] ?? t : t;
  useEffect(() => { if (!tabs.includes(tab)) setTab('overview'); }, [tabs.join(), tab]);
  const url = /^https?:\/\/\S+$/i.test(revision.content.trim().split('\n')[0]);
  ...
  return <article className="detail-pane" aria-label="Selected item">
    <header className="detail-heading"><div className="eyebrow"><KindIcon kind={item.kind} size={14} />{item.kind}<span className="dot">·</span><span title={item.collection ? `In the “${item.collection}” collection. Collections are folders you organise; the lifecycle state is the badge below.` : 'Not in any collection; it shows in the whole library and under Unfiled.'}><Folder size={13} />{item.collection.replaceAll('/', ' / ') || 'Unfiled'}</span></div><div className="title-row"><div><h1>{item.title}</h1>{item.description && <p className="item-lede">{item.description}</p>}</div><button className={`icon-button ${item.favourite ? 'favourited' : ''}`} aria-label={item.favourite ? 'Remove favourite' : 'Add favourite'} onClick={() => void perform(async () => { await api('items.meta', { id: item.id, expect: item.revision, favourite: !item.favourite }); await refresh(); })}><Star size={20} fill={item.favourite ? 'currentColor' : 'none'} /></button></div><div className="detail-meta">{isSource && !item.deletedAt && !['archived', 'rejected'].includes(item.status) ? <button type="button" className="source-chip" onClick={() => changeTab('made')} title="Items an analysis made from this source"><Layers3 size={12} />{made.length} made from it</button> : <Badge status={item.deletedAt ? 'deleted' : ['archived', 'rejected'].includes(item.status) ? item.status : currentApproved ? 'approved' : 'draft'} />}{fromSource && <button type="button" className="source-chip" onClick={() => onSelect(fromSource.id)} title="Open the source this was made from"><FileInput size={12} />From “{fromSource.title}”</button>}<span>Updated {date(item.updatedAt)}</span>{sameTitle && <span className="detail-source" title={`Another item is also called “${item.title}”. This one came from ${sameTitle.full}.`}>from {sameTitle.label}</span>}</div>
      <div className="detail-actions">{item.deletedAt ? <><button className="button primary" onClick={() => void perform(async () => { await api('items.meta', { id: item.id, expect: item.revision, deleted: false }); await refresh(); }, 'Item restored')}><RotateCcw size={15} />Restore</button><button className="button danger-text" onClick={() => onAction('purge')}><Trash2 size={15} />Delete permanently…</button></>
        : isSource ? <>
          {url && <button className="button primary" onClick={() => void perform(() => api('desktop.openItem', { id: item.id }))}><ExternalLink size={15} />Open original</button>}
          <button className={`button ${url ? '' : 'primary'}`} onClick={analyse} title="Run the analysis again; new entries are added beside the earlier ones"><ScanSearch size={15} />Analyze again</button>
    ...
    <div className="detail-scroll" {...scroll}><AgentStatus itemId={item.id} jobs={jobs} onOpen={tabFor} />
      {tab === 'overview' && <>
        ...
        <section className="content-section"><div className="section-heading"><h3>{isSource ? 'Original material' : 'Content preview'}</h3><button className="text-button" aria-pressed={formatted} onClick={() => setFormatted(!formatted)}>{formatted ? 'Raw text' : 'Preview'}</button><button className="text-button" onClick={() => { changeTab('content'); setEditing(true); }}>Open editor <ArrowRight size={13} /></button></div>
          ...
          {formatted ? <Markdown>{revision.content}</Markdown> : <pre className="content-preview">{revision.content}</pre>}</section>
        <section className="content-section"><div className="section-heading"><h3>Provenance</h3><GitBranch size={15} /></div><dl className="metadata-list"><dt>Source</dt><dd>{item.source || 'Captured locally'}</dd><dt>Licence</dt><dd>{item.licence}</dd><dt>Captured</dt><dd>{date(item.createdAt)}</dd><dt>Item ID</dt><dd><code>{item.id}</code></dd>{item.origin && <><dt>{fromSource ? 'Made from source' : 'Derived from'}</dt><dd><button className="text-button" onClick={() => onSelect(item.origin!.itemId)}>{snapshot.items.find(i => i.id === item.origin!.itemId)?.title ?? shortHash(item.origin.itemId)} @ {shortHash(item.origin.revision)} <ArrowRight size={12} /></button></dd></>}</dl><div className="tags">{item.tags.map(tag => <span key={tag}>#{tag}</span>)}</div></section>
        {!isSource && <div className="evidence-grid"><div><strong>{copied}</strong><span>copies observed</span></div><div><strong>{trials.filter(t => t.status === 'completed').length}</strong><span>completed trials</span></div><div><strong>{deployed.filter(d => d.status === 'applied').length}</strong><span>install receipts</span></div></div>}

      </>}
      {tab === 'made' && <>
        <div className="section-heading"><h3>{made.length} item{made.length === 1 ? '' : 's'} made from this source</h3><button className="button" disabled={!made.length} onClick={() => onMadeFrom(item.id)}><Layers3 size={14} />Show in library</button></div>
        <p className="muted">Each keeps a link back here, wherever it is filed; moving entries or deleting their collection does not break it.</p>
...
function MadeList({ items, onSelect, grouped = false }: { items: Item[]; onSelect: (id: string) => void; grouped?: boolean }) {
