...
export function Detail({ jobs, detail, snapshot, providers, sameTitle, installations, onAction, onToggleInstall, refresh, perform, onSelect, onSetup, onCollection }: Props) {
  ...
  return <article className="detail-pane" aria-label="Selected item">
    <header className="detail-heading"><div className="eyebrow"><KindIcon kind={item.kind} size={14} />{item.kind}<span className="dot">·</span><span title={`In the “${item.collection}” collection. Collections are folders you organise; the lifecycle state is the badge below.`}><Folder size={13} />{item.collection}</span>{sameTitle && <><span className="dot">·</span><span className="detail-source" title={`Another item is also called “${item.title}”. This one came from ${sameTitle}.`}>from {sameTitle}</span></>}</div><div className="title-row"><div><h1>{item.title}</h1>{item.description && <p className="item-lede">{item.description}</p>}</div><button className={`icon-button ${item.favourite ? 'favourited' : ''}`} aria-label={item.favourite ? 'Remove favourite' : 'Add favourite'} onClick={() => void perform(async () => { await api('items.meta', { id: item.id, expect: item.revision, favourite: !item.favourite }); await refresh(); })}><Star size={20} fill={item.favourite ? 'currentColor' : 'none'} /></button></div><div className="detail-meta"><Badge status={item.deletedAt ? 'deleted' : ['archived', 'rejected'].includes(item.status) ? item.status : currentApproved ? 'approved' : 'draft'} /><span>Updated {date(item.updatedAt)}</span></div>
