...
export function ItemRow({ item, tab, collectionShown, locations, installations, published }: { item: Item; tab: LibraryTab; collectionShown: boolean; locations: Location[]; installations: Installation[]; published: boolean }) {
  const cols = columns(tab);
  ...
  const subtitle = item.description || (tab === 'recent' || tab === 'favourites' || collectionShown ? '' : item.tags.slice(0, 3).map(t => `#${t}`).join('  '));
