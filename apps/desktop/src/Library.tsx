...
export function ItemRow({ item, tab, collectionShown, locations, installations, published, from }: { item: Item; tab: LibraryTab; collectionShown: boolean; locations: Location[]; installations: Installation[]; published: boolean; from?: string }) {
  const cols = columns(tab);
  ...
  const subtitle = [from ? `from ${from}` : '', item.description || (tab === 'recent' || tab === 'favourites' || collectionShown ? '' : item.tags.slice(0, 3).map(t => `#${t}`).join('  '))].filter(Boolean).join(' · ');
  return <><span className={`item-kind ${item.kind}`}><KindIcon kind={item.kind} size={14} /></span><span className="lib-title"><span className="item-title">{item.title}{item.favourite && <Star size={12} fill="currentColor" />}</span><small className="lib-sub"><span className="lib-narrow-only"><span className="item-kind-label">{item.kind}</span>{collectionShown ? '' : ` · ${item.collection}`}{subtitle ? ' · ' : ''}</span>{subtitle}</small></span>{cols.slice(1).map(c => cell(c.key))}</>;
}
