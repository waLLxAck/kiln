...
import { CollectionsDialog, DeleteCollectionDialog, itemsWithin, MoveItemsDialog } from './Collections';
...
type Dialog = { name: string; workspace?: string; trial?: Trial; itemId?: string; itemIds?: string[]; provider?: ProviderId; targetId?: string; collection?: string; /** Collections dialog: start adding a subfolder here. */ parent?: string } | null;
...
export default function App() {
  ...
  const matching = arrangeItems(tabbed, order, snapshot.usage, pinSources);
  ...
  const collectionEntries = (name: string): MenuEntry[] => {
    ...
    return [
      ...
      { label: 'New subfolder…', icon: <FolderPlus />, shortcut: 'N', onSelect: () => setDialog({ name: 'collections', parent: name }) },
      { label: 'Rename…', icon: <Pencil />, shortcut: 'R', hint: 'Moves its subfolders and items to the new name. Use / to nest it in another collection.', onSelect: () => setDialog({ name: 'collections', collection: name }) },
  ...
  return <div className="app-shell">
    <aside className="sidebar" style={sidebar.style}><div className="brand-row"><button className="brand" onClick={() => navigate('library')}><span className="brand-symbol"><KilnMark /></span><span>Kiln</span></button><button className="icon-button" aria-label="Toggle theme" title="Switch between light and dark" onClick={toggleTheme}>{(theme ?? snapshot.settings.theme) === 'dark' ? <Sun size={15} /> : <Moon size={15} />}</button></div><button className="quick-search" onClick={() => void api('desktop.palette')}><Search size={15} /><span>Quick search</span><kbd>Ctrl K</kbd></button>
      ...
      <div className="sidebar-label collection-label">COLLECTIONS<button className="icon-button" aria-label="Manage collections" onClick={() => setDialog({ name: 'collections' })}><Plus size={13} /></button></div>{snapshot.collections.filter(c => !ancestorsOf(c).some(a => collapsed.includes(a))).map(c => { const parent = snapshot.collections.some(n => n.startsWith(c + '/')), open = !collapsed.includes(c); return <div className="collection-node" key={c} style={{ paddingLeft: depthOf(c) * 14 }}>{nested && (parent ? <button className="collection-toggle" aria-label={`${open ? 'Collapse' : 'Expand'} ${c}`} aria-expanded={open} onClick={() => toggleCollapsed(c)}>{open ? <ChevronDown size={13} /> : <ChevronRight size={13} />}</button> : <span className="collection-toggle" aria-hidden="true" />)}<button className={`nav-item ${collection === c && libraryView ? 'active' : ''}`} onClick={() => openCollection(c)} onContextMenu={event => { event.preventDefault(); setCollectionMenu({ x: event.clientX, y: event.clientY, name: c }); }} title={parent ? `Show “${c}” and its subfolders in the library` : `Show only “${c}” in the library`}><Folder size={16} /><span>{leafOf(c)}</span><small>{live.filter(i => isWithin(i.collection, c) && !hidden.includes(i.status)).length}</small></button></div>; })}{(unfiled > 0 || collection === UNFILED) && <button className={`nav-item ${collection === UNFILED && libraryView ? 'active' : ''}`} onClick={() => openCollection(UNFILED)} title="Items outside every collection. They also show in the whole library."><Inbox size={16} /><span>Unfiled</span><small>{unfiled || ''}</small></button>}<button className={`nav-item ${section === 'archive' ? 'active' : ''}`} onClick={() => navigate('archive')}><Archive size={16} /><span>Archive</span><small aria-hidden="true">{live.filter(i => hidden.includes(i.status)).length || ''}</small></button><button className={`nav-item ${section === 'trash' ? 'active' : ''}`} onClick={() => navigate('trash')}><Trash2 size={16} /><span>Trash</span><small aria-hidden="true">{snapshot.items.filter(i => i.deletedAt).length || ''}</small></button>
    ...
    {dialog?.name === 'collections' && <CollectionsDialog names={snapshot.collections} items={snapshot.items} renaming={dialog.collection} subfolderOf={dialog.parent} onClose={() => setDialog(null)} onDone={async () => { const renamed = dialog.collection && collection && isWithin(collection, dialog.collection); await refresh(); if (renamed) setCollection(''); }} onDelete={name => setDialog({ name: 'delete-collection', collection: name })} onOpen={openCollection} />}
