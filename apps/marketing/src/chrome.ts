...
import { home, installer, issues, releases, repository, support } from './content';
import { logo } from './ui';
...
export function siteHeader(page: Page) {
  ...
  return `<header class="site-header"><a class="brand" href="${page === 'home' ? '#main' : home}">${logo(20)}Kiln</a>
    <nav aria-label="Main navigation"><a href="${at('#folders')}">My folders</a><a href="${at('#new-skills')}">New skills</a><a href="${support}" class="header-support"${page === 'support' ? ' aria-current="page"' : ''}>Support</a><a href="${at('#download')}" class="header-download">Download</a></nav></header>`;
}
...
export function siteFooter() {
  return `<footer class="site-footer">
    ...
    <nav aria-label="Footer"><a href="${installer}">Download for Windows</a><a href="${releases}">All releases</a><a href="${repository}">Source on GitHub</a><a href="${issues}">Report an issue</a><a href="${support}">Support Kiln</a></nav>
  </footer>`;
}
