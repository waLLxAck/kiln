// Pointer dragging (mouse, pen and touch): the item lifts off the page as a ghost that tilts with the pointer, the drop zone
// arms and lights up, the page auto-scrolls near the screen edge, and a drop is swallowed by the zone with a little gulp.
// The keyboard and tap fallback ("Add to Kiln") flies the same ghost along an arc.
