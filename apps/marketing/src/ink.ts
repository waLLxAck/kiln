// The red pen: margin notes that sit next to what they point at (wide layout) or between the blocks (stacked layout), plus
// marks (circles, loops, boxes, underlines) and arrows drawn in one SVG over the sheet. Everything draws on once it's in view.
import { prefersReducedMotion } from './motion';
import { arrow, ellipse, line, reseed } from './rough';

type Mark = 'circle' | 'loop' | 'box' | 'underline' | 'none';
...
export type InkNote = {
  ...
  /** Stacked layout only: keep the note's space from the start (still invisible), so the layout doesn't jump when it arrives. */
  reserve?: boolean;
};
...
const path = (d: string, i = 0) => `<path d="${d}" class="pen-stroke" pathLength="1" style="--i:${i}"/>`;

function markPath(mark: Mark, r: Rect, seed = 1) {
  reseed(seed);
...
export function createInk(sheet: HTMLElement, notes: InkNote[], marks: InkMark[], isWide: () => boolean) {
  ...
  function place() {
    ...
    for (const note of notes) {
      ...
      if (!el) continue;
      el.hidden = !(live(note) || (!wide && note.reserve && !note.until));
      if (el.hidden || !wide) { el.style.left = el.style.top = el.style.width = ''; continue; }
  ...
  function draw() {
    const w = sheet.offsetWidth, h = sheet.offsetHeight, wide = isWide();
    ...
    notes.forEach((note, n) => {
      ...
      // Stacked: only the note nearest its target draws an arrow, so no arrow runs through another note.
      ...
      const blocked = !wide && (p.y > t.y ? sib('previousElementSibling') : sib('nextElementSibling'));
      const near = wide || (!blocked && Math.abs((p.y + p.h / 2) - (t.y + t.h / 2)) < 240);
      reseed(301 + n * 13);
      const a = note.noArrow || !near ? '' : arrowPath(p, markBox, note.bend ?? (n % 2 ? 1 : -1));
      out += `<g class="pen-mark${on.has(note.id) ? ' is-on' : ''}" data-mark="${note.id}">${m ? path(m, 0) : ''}${a ? path(a, 1) : ''}</g>`;
    });
    marks.forEach((mark, n) => {
      ...
      if (mark.mark === 'link') {
        ...
        if (!wide || !visible(from) || !visible(to)) return;
      ...
      out += `<g class="pen-mark${on.has(mark.id) ? ' is-on' : ''}" data-mark="${mark.id}">${path(d)}</g>`;
    });
    svg.innerHTML = out;
  }
  ...
  function arm() {
    ...
    for (const { id, el: found } of all) {
      ...
      seen.add(id);
      if (prefersReducedMotion() || !observer) { turnOn(id); continue; }
