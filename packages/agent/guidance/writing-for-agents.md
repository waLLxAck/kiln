...
- An **input** — a `{{placeholder}}` the human fills — pushes a lookup onto the human. The executing agent usually runs inside the target repository, so leave the project name, stack, layout, conventions and commands to the environment, and ask only for what the human alone holds: an intent, a decision, a reference they supply. Keep inputs few, and write the document so it still reads when one is left unfilled.
