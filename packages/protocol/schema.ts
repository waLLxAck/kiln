...
export type UpdateStatus = { current: string; source: string; /** setting: chosen in Settings; build: the release folder of the repository this build came from; off: checks disabled; none: nothing to watch. */ sourceKind: 'setting' | 'build' | 'off' | 'none'; packaged: boolean; available: { version: string; path: string } | null; stage: UpdateStage; /** Git commit this build was made from, when known. */ commit: string; error?: string };
...
export interface Bridge { call<T = unknown>(method: string, args?: unknown): Promise<T>; }
