...
import { sharedTitleIds, sourceLabel, titleCollisions } from '../apps/desktop/src/item-source';
...
test('only items sharing a kind and title get a source label, and only when the sources differ', () => {
  ...
  assert.deepEqual(Object.fromEntries(labels), { a: '~/.claude/skills/synced/0c6f/skill-creator', b: '~/.codex/skills/.system/skill-creator' });
});
...
test('imported items whose shared source is only a folder name are told apart by the origin kept on this machine', () => {
  ...
  assert.deepEqual(Object.fromEntries(titleCollisions(items, '/home/ada', origins)), { a: '~/.claude/skills/synced/0c6f/skill-creator', b: '~/.codex/skills/.system/skill-creator' });
});
