...
**Sort** in the tools row orders the list: **Recently added** (the default everywhere), **Recently updated**, **Most copied** (copies made from Kiln), **Most used** (copies, opens, tests and agent use Kiln has seen), **Title A–Z**, or **Custom order**, the hand-arranged order that the up and down arrows change. Clicking a column heading sorts by it too. Each view remembers its own order. Inside a collection, its **sources** always come first, sorted among themselves; across the whole library they mix in with everything else.
